...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.action === "getClosedSessions") {
        const closedSessions = Object.entries(sesiones)
            .filter(([_, session]) => !session.abierta)
            .map(([_, session]) => session.recorrido);

        sendResponse({ sessions: closedSessions });
    }
});
...
function sendDataToFlask(tabId, sesion) {
  fetch('http://127.0.0.1:5000/api', { // Cambia la URL según tu servidor Flask
    method: 'POST',
    headers: {
